    ...
    for r = 1:NumRes
        ...
        if Temp_Nroutes > 0
            ...
            if strcmp(Simulation.MergeModel,'demfifo')
                Reservoir(r).EntrySupplyTime = zeros(1,Temp_Nextnodes);
                Reservoir(r).ExitSupplyTime = zeros(1,Temp_Nextnodes);
            else
...
for r = Global.OriResList
    ...
    if ~isempty(Temp_indexes)
        if strcmp(Simulation.MergeModel,'demfifo') % && length(Reservoir(r).EntryNodesIndex) == 1
            ...
            i_r = Route(iroute).ResRouteIndex(r);
            Temp_time2 = 0;%Reservoir(r).LastEntryTime + 1/Reservoir(r).EntryNodeSupplyPerRoute(i_r);
...
while CurrentTime < Temp_EndTime
    ...
    for r = NextEvent.ResList % loop only on reservoirs where accumulation changed
        ...
        Temp_criticalline = 0;
        for i_m = 1:NumModes
            Temp_criticalline = Temp_criticalline - Reservoir(r).CurrentAcc(i_m)/(Temp_scf*Reservoir(r).CritAcc(i_m));
        end
        Temp_criticalline = Temp_criticalline + 1;
    ...
    for r = NextEvent.ResList % loop only on reservoirs where accumulation changed
        ...
        for i_r = 1:Temp_Nroutes
            ...
            if strcmp(MacroNode(inode).Type, 'externalentry') || strcmp(MacroNode(inode).Type, 'origin')
        ...
        elseif strcmp(Simulation.MergeModel,'demfifo')
            ...
            for i_e = 1:length(Reservoir(r).EntryRoutesIndexPerNode)
                ...
                Reservoir(r).EntrySupplyTime(i_e) = Reservoir(r).LastEntryTime + 1/Temp_supplytimes; % global entry supply time per node
            end
        else
