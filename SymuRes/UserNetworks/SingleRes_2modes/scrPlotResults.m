...
%--------------------------------------------------------------------------
iresu = 1;
Results(iresu).Network = 'SingleRes_2modes'; % Choice of a network defined by user
Results(iresu).Solver = 1; % Choice of the solver. 1: accbased / 2: tripbased
Results(iresu).Name = 'SC32'; % Simulation name
...
ResuList = 1; % list of results
AccRange = [0 1000]; % [veh]
ProdRange1 = [0 5000]; % mode 1 [veh.m/s]
ProdRange2 = [0 600]; % mode 2 [veh.m/s]
Nfig = length(ResList); % number of subfigures
Nresu = length(ResuList); % number of results
